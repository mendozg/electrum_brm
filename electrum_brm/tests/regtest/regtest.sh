...
if [[ $1 == "unixsockets" ]]; then
    ...
    ./run_electrum_brm --regtest daemon -d --rpcsock=unix # Start daemon with unix domain socket
    ./run_electrum_brm --regtest stop # Errors if it can't connect
    # Test custom socket path
    f=$(mktemp --dry-run)
    ./run_electrum_brm --regtest daemon -d --rpcsock=unix --rpcsockpath=$f
    [ -S $f ] # filename exists and is socket
    ./run_electrum_brm --regtest stop
    rm $f # clean up
    # Test for regressions in the ordinary TCP functionality.
    ./run_electrum_brm --regtest daemon -d --rpcsock=tcp
    ./run_electrum_brm --regtest stop
fi
